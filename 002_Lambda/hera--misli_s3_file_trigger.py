...
def cp_object(bucket,source_key,destination_path):
    s3 = boto3.client('s3')
...
def check_week_files_number(bucket, depentents):
    s3 = boto3.client('s3')    
    ...
    for record in event['Records']:
        ...
        if my_record['Is_Formatted'] == True:
            ...
            if status == 200 or status == 0:                                                 
                ...
                # eger sikinti yoksa weekly ve mountly icin diger periodlara aktarım oaliblir
                if my_record['folder'] == 'data' and my_record['period'] == 'daily':
                    a_date = datetime.date(int(my_record['year']), int(my_record['month']), int(my_record['day']))
                    my_record["week_of_year"] = a_date.isocalendar()[1]
                    my_record["day_of_week"] = a_date.weekday()+1
                    destination_path = 'data/weekly/{state}/{root_name}/{year}/{week_of_year}/{day_of_week}_{table_name}_{year}{month}{day}--{its_rnd_str}.parquet'.format(**my_record) 
                    cp_object('misli.bet.data',  my_record['key'], destination_path)
